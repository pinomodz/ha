...
try {
		...
		cmhit.push(command)
        mess = {
			wait: 'Tunggu Sebentar, Kalo Pada Nyepam Bakal Di Disconect Sama Owner Gw.',
			success: 'Nih Jangan Lupa Donasi Ya Kack:)',
			error: {
				stick: 'Cannot access videos!',
				Iv: 'Link Nya Salah Tod',
                api: 'Api Key Habis Owner Mau Beli Gak Ada Yg Donasi:('
			},
			only: {
		...
		const katalog = (teks) => {
             res = denz.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_©Z A M  B O T_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             denz.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = denz.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/FJdtdDteQkkBSXkAl44sey', "groupName": `${NamaBot}`, "footerText": "*_©Z A M  B O T_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
...
denz.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
 ...
 if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		  ...
		  sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "*_©Z A M  B O T_*", [{buttonId: 'simi', buttonText: {displayText: 'Oke'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
			}
		}
		if (mek.key.remoteJid.endsWith('@g.us') && offline) {
			if (!mek.key.fromMe){
		...
		const reply = (teks) => {
			denz.sendMessage(from, teks, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} - ${week} ${weton} - ${calender}`,body:"",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://wa.me/6285643260438?text=Assalamualaikum`}}})
		}
		const reply2 = (teks) => {
...
- Hari : ${week} ${weton}
- Tanggal : ${calender}`
sendButLocation(from, `${menu}`, "Z A M  B O T 右", {jpegThumbnail:gambar,name:""}, [
{buttonId:`allmenu`,buttonText:{displayText:'📑SHOW MENU'},type:1},
{buttonId:`owner`,buttonText:{displayText:'👑OWNER'},type:1},
{buttonId:`script`,buttonText:{displayText:'🔖SCRIPT'},type:1}
...
└─────────────────❒`
sendButLocation(from, `${menu}`, "Z A M  B O T 右", {jpegThumbnail:gambar,name:""}, [{buttonId:`owner`,buttonText:{displayText:'👑OWNER'},type:1},{buttonId:`script`,buttonText:{displayText:'📒SCRIPT'},type:1}], {contextInfo: { mentionedJid: [otod]}})
break
case 'command':
 stod = `${sender}`
 listMsg = {
 buttonText: 'LIST MENU',
 footerText: '*_©Z A M  B O T_*',
...
client.browserDescription = ['N I Z A M','Desktop','3.0']
...
break
		...
		case 'sc':
		case 'sourcecode':
		anu =`MAU SC NYA? BUY MEK MURAH KOK:D`
...
Giliran = @${tty.player1.split('@')[0]}`
 denz.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     denz.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer ZAM BOT",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./denz.jpg'),sourceUrl:"https://wa.me/6285643260438?text=Assalamualaikum, Save [NAMA KAMU]"}}})
	}
    }
    }
    }
